...
public class MapFunctions
{
    ...
    public static SquareArray<int> GenerateArray(int width, int height, bool empty)
    {
        SquareArray<int> map = new SquareArray<int>(width);
        for (int x = 0; x < map.SideLength + 1; x++)
        {
            for (int y = 0; y < map.SideLength + 1; y++)
            {
                if (empty)
                {
                    map[x, y] = 0;
    ...
    public static void RenderMap(SquareArray<int> map, Tilemap tilemap, TileBase tile)
    {
        tilemap.ClearAllTiles(); //Clear the map (ensures we dont overlap)
        for (int x = 0; x < map.SideLength + 1; x++) //Loop through the width of the map
        {
            for (int y = 0; y < map.SideLength + 1; y++) //Loop through the height of the map
            {
    ...
    public static void RenderMapWithOffset(SquareArray<int> map, Tilemap tilemap, TileBase tile, Vector2Int offset)
    {
        for (int x = 0; x < map.SideLength + 1; x++)
        {
            for (int y = 0; y < map.SideLength + 1; y++)
    ...
    /// <param name="tilemap">The tilemap to draw on</param>
    /// <param name="tile">The tile to draw with</param>
    public static IEnumerator RenderMapWithDelay(SquareArray<int> map, Tilemap tilemap, TileBase tile)
    {
        for (int x = 0; x < map.SideLength + 1; x++)
        {
            for (int y = 0; y < map.SideLength + 1; y++)
    ...
    public static void UpdateMap(SquareArray<int> map, Tilemap tilemap)
    {
        for (int x = 0; x < map.SideLength + 1; x++)
        {
            for (int y = 0; y < map.SideLength + 1; y++)
    ...
    public static SquareArray<int> PerlinNoise(SquareArray<int> map, float seed)
    {
        int newPoint;
        ...
        for (int x = 0; x < map.SideLength + 1; x++)
        {
            newPoint = Mathf.FloorToInt((Mathf.PerlinNoise(x, seed) - reduction) * map.SideLength + 1);

            //Make sure the noise starts near the halfway point of the height
            newPoint += (map.SideLength + 1 / 2);
    ...
    public static SquareArray<int> PerlinNoiseSmooth(SquareArray<int> map, float seed, int interval)
    {
        ...
        if (interval > 1)
        {
            ...
            //Generate the noise
            for (int x = 0; x < map.SideLength + 1; x += interval)
            {
                newPoint = Mathf.FloorToInt((Mathf.PerlinNoise(x, (seed * reduction))) * map.SideLength + 1);
                noiseY.Add(newPoint);
                noiseX.Add(x);
    ...
    public static SquareArray<int> PerlinNoiseCave(SquareArray<int> map, float modifier, bool edgesAreWalls)
    {
        int newPoint;
        for (int x = 0; x < map.SideLength + 1; x++)
        {
            for (int y = 0; y < map.SideLength + 1; y++)
            {
                ...
                if (edgesAreWalls && (x == 0 || y == 0 || x == map.SideLength + 1 - 1 || y == map.SideLength + 1 - 1))
                {
    ...
    public static SquareArray<int> RandomWalkTop(SquareArray<int> map, float seed)
    {
        //Seed our random
        System.Random rand = new System.Random(seed.GetHashCode());

        //Set our starting height
        int lastHeight = Random.Range(0, map.SideLength + 1);
        ...
        for (int x = 0; x < map.SideLength + 1; x++)
        {
            ...
            else if (nextMove == 1 && lastHeight < map.SideLength + 1 - 2)
            {
                lastHeight++;
            }
    ...
    public static SquareArray<int> RandomWalkTopSmoothed(SquareArray<int> map, float seed, int minSectionWidth)
    {
        //Seed our random
        System.Random rand = new System.Random(seed.GetHashCode());
        ...
        for (int x = 0; x < map.SideLength; x++)
        {
            ...
            else if (nextMove == 1 && lastHeight < map.SideLength + 1 && sectionWidth > minSectionWidth)
            {
                lastHeight++;
                sectionWidth = 0;
            }
    ...
    public static SquareArray<int> RandomWalkCave(SquareArray<int> map, float seed, int requiredFloorPercent)
    {
        //Seed our random
        System.Random rand = new System.Random(seed.GetHashCode());

        //Define our start x position
        int floorX = rand.Next(1, map.SideLength + 1 - 1);
        //Define our start y position
        int floorY = rand.Next(1, map.SideLength + 1 - 1);
        //Determine our required floorAmount
        int reqFloorAmount = ((map.SideLength + 1 * map.SideLength + 1) * requiredFloorPercent) / 100;
        ...
        while (floorCount < reqFloorAmount)
        {
            ...
            switch (randDir)
            {
                case 0: //Up
                    ...
                    if ((floorY + 1) < map.SideLength + 1 - 1)
                    {
                        //Move the y up one
                        floorY++;
                ...
                case 2: //Right
                    //Ensure that the edges are still tiles
                    if ((floorX + 1) < map.SideLength + 1 - 1)
                    {
    ...
    /// <returns>The modified map array</returns>
    public static SquareArray<int> RandomWalkCaveCustom(SquareArray<int> map, float seed, int requiredFloorPercent)
    {
        //Seed our random
        System.Random rand = new System.Random(seed.GetHashCode());

        //Define our start x position
        int floorX = Random.Range(1, map.SideLength + 1 - 1);
        //Define our start y position
        int floorY = Random.Range(1, map.SideLength + 1 - 1);
        //Determine our required floorAmount
        int reqFloorAmount = ((map.SideLength + 1 * map.SideLength + 1) * requiredFloorPercent) / 100;
        //Used for our while loop, when this reaches our reqFloorAmount we will stop tunneling
        int floorCount = 0;
        ...
        while (floorCount < reqFloorAmount)
        {
            ...
            switch (randDir)
            {
                case 0: //North-West
                    ...
                    if ((floorY + 1) < map.SideLength + 1 && (floorX - 1) > 0)
                    {
                        //Move the y up 
                        floorY++;
                        //Move the x left
                ...
                case 2: //North-East
                    //Ensure we don't go off the map
                    if ((floorY + 1) < map.SideLength + 1 && (floorX + 1) < map.SideLength + 1)
                    {
                        //Move the y up
                        floorY++;
                        //Move the x right
                ...
                case 4: //South-East
                    //Ensure we don't go off the map
                    if ((floorY - 1) > 0 && (floorX + 1) < map.SideLength + 1)
                    {
                        //Move the y down
                        floorY--;
                        //Move the x right
    ...
    public static SquareArray<int> DirectionalTunnel(SquareArray<int> map, int minPathWidth, int maxPathWidth, int maxPathChange, int roughness, int windyness)
    {
        ...
        for (int y = 1; y < map.SideLength + 1; y++)
        {
            ...
            if (rand.Next(0, 100) > windyness)
            {
                ...
                if (x > (map.SideLength + 1 - maxPathWidth))
                {
                    x = map.SideLength + 1 - maxPathWidth;
                }
    ...
    /// <returns>The modified map array</returns>
    public static SquareArray<int> GenerateCellularAutomata(int width, float seed, int fillPercent, bool edgesAreWalls)
    {
        ...
        for (int x = 0; x < map.SideLength + 1; x++)
        {
            for (int y = 0; y < map.SideLength + 1; y++)
            {
                if (edgesAreWalls && (x == 0 || x == map.SideLength + 1 - 1 || y == 0 || y == map.SideLength + 1 - 1))
                {
    ...
    /// <returns>The modified map array</returns>
    public static SquareArray<int> SmoothVNCellularAutomata(SquareArray<int> map, bool edgesAreWalls, int smoothCount)
    {
        for (int i = 0; i < smoothCount; i++)
        {
            for (int x = 0; x < map.SideLength + 1; x++)
            {
                for (int y = 0; y < map.SideLength + 1; y++)
                {
                    ...
                    if (edgesAreWalls && (x == 0 || x == map.SideLength + 1 - 1 || y == 0 || y == map.SideLength + 1))
                    {
	...
	static int GetVNSurroundingTiles(SquareArray<int> map, int x, int y, bool edgesAreWalls)
    {
    ...
    /// <param name="smoothCount">The amount we will loop through to smooth the array</param>
    /// <returns>The modified map</returns>
    public static SquareArray<int> SmoothMooreCellularAutomata(SquareArray<int> map, bool edgesAreWalls, int smoothCount)
    {
        for (int i = 0; i < smoothCount; i++)
        {
            for (int x = 0; x < map.SideLength + 1; x++)
    ...
    static int GetMooreSurroundingTiles(SquareArray<int> map, int x, int y, bool edgesAreWalls)
    {
        ...
        for (int neighbourX = x - 1; neighbourX <= x + 1; neighbourX++)
        {
            ...
            {
                if (neighbourX >= 0 && neighbourX < map.SideLength + 1 && neighbourY >= 0 && neighbourY < map.SideLength + 1)
                {
