...
public class LandMeshGenerator : MonoBehaviour
{
    ...
    Dictionary<Terrain, TerrainMapTile> terrainTileLookup)
    {
        ...
        float texCordToTileCordScale = (float)mapData.TerrainMap.SideLength / (float)sedimentTextureSize;
        float TileCordToTexCordScale = (float)sedimentTextureSize / (float)mapData.TerrainMap.SideLength;

        SquareArray<TerrainChannels> alphaMap = new SquareArray<TerrainChannels>(sedimentTextureSize);
        int numThreads = 16;
        TerrainChannels[] neighborChannels = new TerrainChannels[numThreads];
        ...
        ArrayUtilityFunctions.ForMTWithThreadID(alphaMap.SideLength, (threadId, x, y) =>
        {
            ...
            sedimentColors[x, y] = MixSedimentColors(ac, terrainTileLookup, mapData.SedimentMap[sedimentPos]);
        }, numThreads);
        ...
        for(int i = 0; i < (int)Terrain.Max; i++)
        {
            ...
            if(terrainTileLookup.ContainsKey((Terrain)i))
            {
                ...
                Color color = Color.Lerp(
                    terrainTileLookup[(Terrain)i].SimpleDisplayColor,
                    terrainTileLookup[(Terrain)i].SecondaryDisplayColor,
                    colorWeight);
                sum += color * weight;
            }
        }
    ...
    private Color32 MixSedimentColors(TerrainChannels channels, Dictionary<Terrain, TerrainMapTile> terrainTileLookup, float a)
    {
        Color sum = new Color();
        for (int i = 0; i < (int)Terrain.Max; i++)
        {
            ...
            if (terrainTileLookup.ContainsKey((Terrain)i))
            {
                sum += terrainTileLookup[(Terrain)i].SedimentColor * weight;
            }
        }
